...
def spherical_virtual_cameras(count: int, radius: float, image_size: tuple[int, int],
    ...
    """Create orbit cameras that look at the origin."""
    if count < 1:
    ...
    for index in range(count):
        ...
        rotation = look_at_rotation(position, np.zeros(3, dtype=np.float64))
...
def look_at_rotation(position: np.ndarray, target: np.ndarray) -> np.ndarray:
